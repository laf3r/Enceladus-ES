...
void Mfeistel_cipher(uint8_t mode, uint32_t* block32b_1, uint32_t* block32b_2, uint32_t* keys32b);
void Mround_of_Mfeistel_cipher(uint32_t* block32b_1, uint32_t* block32b_2, uint32_t* keys32b, uint8_t round);

uint32_t Msubstitution_table(uint32_t block32b, uint8_t sbox_row);
void Msubstitution_table_by_4bits(uint8_t* blocks4b, uint8_t sbox_row);

void Msplit_256bits_to_32bits(uint8_t* key256b, uint32_t* keys32b);
void Msplit_64bits_to_8bits(uint64_t block64b, uint8_t* blocks8b);
void Msplit_32bits_to_8bits(uint32_t block32b, uint8_t* blocks4b);

uint64_t Mjoin_32bits_to_64bits(uint32_t block32b_1, uint32_t block32b_2);
uint64_t Mjoin_8bits_to_64bits(uint8_t* blocks8b);
uint32_t Mjoin_4bits_to_32bits(uint8_t* blocks4b);

static inline void Mprint_array(uint8_t* array, size_t length);
static inline void Mprint_array(uint64_t x, register uint64_t Nbit);

// 1 | 4 -> 0xC
static const uint8_t MSbox[8][16] = {
...
void Mfeistel_cipher(uint8_t mode, uint32_t* block32b_1, uint32_t* block32b_2, uint32_t* keys32b) {
    ...
    case 'E': case 'e': {
        ...
        for (uint8_t round = 0; round < 24; ++round)
            Mround_of_Mfeistel_cipher(block32b_1, block32b_2, keys32b, round);
...
void Mround_of_Mfeistel_cipher(uint32_t* block32b_1, uint32_t* block32b_2, uint32_t* keys32b, uint8_t round) {
    ...
    // RES = RES -> MSbox
    result_of_iter = Msubstitution_table(result_of_iter, round % 8);
...
uint32_t Msubstitution_table(uint32_t block32b, uint8_t sbox_row) {
    ...
    Msplit_32bits_to_8bits(block32b, blocks4bits);
    Msubstitution_table_by_4bits(blocks4bits, sbox_row);
    return Mjoin_4bits_to_32bits(blocks4bits);
}
...
void Msubstitution_table_by_4bits(uint8_t* blocks4b, uint8_t sbox_row) {
    ...
    for (uint8_t i = 0; i < 4; ++i) {
        ...
        block4b_1 = MSbox[sbox_row][blocks4b[i] & 0x0F];

        // 10101100 >> 4 = 00001010
        // [example get from table] 1010 -> 0111
        block4b_2 = MSbox[sbox_row][blocks4b[i] >> 4];
...
void Msplit_64bits_to_32bits(uint64_t block64b, uint32_t* block32b_1, uint32_t* block32b_2) {
...
void Msplit_32bits_to_8bits(uint32_t block32b, uint8_t* blocks8b) {
    for (uint8_t i = 0; i < 4; ++i) {
...
size_t GOST_28147(uint8_t* to, uint8_t mode, uint8_t* key256b, uint8_t* from, size_t length) {
    ...
    Msplit_256bits_to_32bits(key256b, keys32b);

    for (size_t i = 0; i < length; i += 8) {
        Msplit_64bits_to_32bits(
            Mjoin_8bits_to_64bits(from + i),
            &N1, &N2
        );
        Mfeistel_cipher(mode, &N1, &N2, keys32b);
        Msplit_64bits_to_8bits(
            Mjoin_32bits_to_64bits(N1, N2),
            (to + i)
        );
    }
